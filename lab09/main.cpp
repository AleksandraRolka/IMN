...
double pow_nabla_T(gsl_vector* T, int l )
{
	double il_rozn = ( gsl_vector_get(T,l+1) -2 * gsl_vector_get(T,l) + gsl_vector_get(T,l-1) ) / pow( delta, 2. ) + ( gsl_vector_get(T,l+n_x+1) -2 * gsl_vector_get(T,l) + gsl_vector_get(T,l-n_x-1) ) / pow( delta, 2. );
	
	return il_rozn;
}

void solve_heatDiffusion_equation()
{
	
	FILE *file_T = fopen( "T.dat", "w");
	FILE *file_pow_nabla_T = fopen( "pow_nabla_T.dat", "w");
	
	
	// utworzyć macierze A[N][N], B[N][N] oraz wektor c[N] i wypełnić je zgodnie z wzorami
	...
	// warunki początkowe wektora T
	i = 0;
	for(int j = 0; j < n_y; j++)     	// lewy brzeg
	{
		l = i + j * ( n_x + 1 );
		gsl_vector_set( T, l, T_A);
	}
	i = n_x;
	for(int j = 0; j < n_y; j++)     	// prawy brzeg
	{
		l = i + j * ( n_x + 1 );
		gsl_vector_set( T, l, T_C);
	}
	for(i = 1; i <= n_x - 1; i++){	 	// pozostały obszar
        for(int j = 0; j <= n_y; j++){
			l = i + j * ( n_x + 1 );
		...
		if( it == 100 || it == 200 || it == 500 || it == 1000 || it == 2000 )
		{
			for(int i = 1; i < n_x; i++){
				for(int j = 1; j < n_y; j++){
					...
					fprintf(file_pow_nabla_T, "%d\t %d\t %d\t %g \n", it, i * delta, j * delta, pow_nabla_T(T,l) );
				}
				fprintf(file_T, "\n");
				fprintf(file_pow_nabla_T, "\n");
			}
			fprintf(file_T, "\n");
			fprintf(file_pow_nabla_T, "\n");
		}
